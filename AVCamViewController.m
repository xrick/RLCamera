...
@synthesize txtSend,statusLed,btnFlash;
...
-(IBAction)SetFlashLight:(id)sender
{
    ...
    if(!statusLed)
    {
        ...
        statusLed = YES;
        [btnFlash setTitle:@"NightMode: ON" forState:UIControlStateNormal];
    }
    else
    {
        [device setTorchMode:AVCaptureTorchModeOff];
        statusLed = NO;
        [btnFlash setTitle:@"NightMode: OFF" forState:UIControlStateNormal];
    }
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ([txtSend isFirstResponder] && [touch view] != txtSend) {
...
{
    if([txtSend.text isEqualToString:@"Your text here!"])
    {
        txtSend.text = @"";
    }
}
...
-(void)openMenu
{
    ...
    NSString* someText = self.txtSend.text;
...
- (void)checkDeviceAuthorizationStatus
{
	...
	[AVCaptureDevice requestAccessForMediaType:mediaType completionHandler:^(BOOL granted) {
		...
		else
		{
			...
			dispatch_async(dispatch_get_main_queue(), ^{
				[[[UIAlertView alloc] initWithTitle:@"Walk!"
											message:@"Walk doesn't have permission to use Camera, please change privacy settings"
										   delegate:self
